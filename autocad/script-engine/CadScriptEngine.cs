...
namespace AutoCADMCP.ScriptEngine
{
    ...
    /// </summary>
    public class CadScriptEngine : ICadCommand
    {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                // Roslyn's generated submissions use their own loader. Share
                // these exact instances there too, or globals/API casts would
                // fail even though the compiler itself is correctly isolated.
                using var assemblyLoader = new InteractiveAssemblyLoader();
                foreach (var dependency in new[]
                {
                    typeof(CadScriptGlobals).Assembly,
                    typeof(ICadCommand).Assembly,
                    typeof(Database).Assembly,
                    typeof(Document).Assembly,
                    typeof(Application).Assembly,
                }.Distinct())
                    assemblyLoader.RegisterDependency(dependency);
